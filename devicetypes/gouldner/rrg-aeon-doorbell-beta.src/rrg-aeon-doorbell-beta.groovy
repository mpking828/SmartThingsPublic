...
metadata {
    definition(name: "RRG Aeon Doorbell Beta", namespace: "gouldner", author: "Robert Vandervoort") {
        capability "Actuator"
        capability "Alarm"
        capability "Battery"
        capability "Switch"
...
def zwaveEvent(physicalgraph.zwave.commands.associationv2.AssociationReport cmd) {
    cmd.nodeId.each({
        log.debug "AssociationReport: '${cmd}', hub: '$zwaveHubNodeId' reports nodeId: '$it' is associated in group: '${cmd.groupingIdentifier}'"
    })
}
...
def configure() {
    if (state.debug) {
        ...
        } else if (state.debug) log.debug "configuration being sent to ${device.displayName}"
    }
